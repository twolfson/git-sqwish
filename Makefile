...
demo:
	...
	@echo 'PS1="$$ "'
	@echo 'git init'
	@echo 'echo "<?xml>" > sitemap.xml'
	@echo 'git commit -m "Initial comit" -- sitemap.xml'
	@echo '# DEMO START'
	@echo '# On a development branch'
	@echo 'git branch'
	@echo '# Make normal changes'
	@echo 'echo "<p>Email us</p>" > support.html'
	@echo 'git commit -m "Created support page" -- support.html'
	@echo '# Introduce conflictable code'
	@echo 'echo "<xml>support.html</xml>" >> sitemap.xml'
	@echo 'git commit -m "Added support page to sitemap" -- sitemap.xml'
	@echo '# Sync with master via `git merge` (as opposed to `git rebase`)'
