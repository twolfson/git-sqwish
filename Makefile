...
demo:
	...
	@echo '### DEMO START ###'
	@echo '##################'
	@echo '\033[3;# On a development branch with normal and conflicting changes\033[0;'
	@echo 'git log --all --graph --decorate --oneline'
	@echo '\033[3;# Sync with master (as opposed to `git rebase`)\033[0;'
	@echo 'git merge master \033[3;# Interchangable with `git pull\033[0;`'
	@echo 'echo "DELETE ME"; git checkout master -- .; git add -A; git commit -m "Merge branch '"'master'"' into dev/add.support.page"'
	@echo 'git mergetool -y; git commit \033[3;# Deal with *one* set of merge conflicts\033[0;'
	@echo '\033[3;# Squash changes\033[0;'
	@echo 'git sqwish master --message "Added support page"'
	@echo '\033[3;# History is preserved on original branch, changes are sqwished to one commit\033[0;'
